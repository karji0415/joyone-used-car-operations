...
> 本文保留早期本地工具的历史试用记录。下列指标不用于证明当前 Agent/API 模拟演示的效果；最新功能范围见 [精简 PRD](prd-lite.md)。
...
## 2026-09-12：从本地工具到统一业务助手入口

本轮把表格核查、问题跟进、人工复核和记录整理为连续流程。Agent 对话收进右上角“询问 JOYone”，左侧仅保留接入中心，减少重复入口。

接入中心展示链接入口、网页内嵌、API、Tool / MCP 四种方式。API 按选择服务、模拟申请、查看或导出示例文档、测试调用组织；Agent 使用已有聊天服务入口的产品思路。公开演示使用预设回复和虚构配置，没有连接公司服务。

当前阶段优先验证核查结果是否可解释、人工确认是否完整、任务是否可追踪。Agent 和接口接入的业务收益需要真实授权与试点后单独验证，不沿用早期版本的效率数字。
